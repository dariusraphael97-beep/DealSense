...
function ScoreRing({ score, size = 180 }: { score: number; size?: number }) {
  ...
  // Full arc for both track and progress — progress revealed via strokeDashoffset
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="relative z-[1]">
        ...
        {/* Progress arc */}
        <path
          d={fullPath} fill="none" stroke="url(#scoreGrad)" strokeWidth={strokeW} strokeLinecap="round"
        ...
        />
        {/* Tip indicator — conditionally rendered after arc animation completes */}
        {showDot && (
...
function ResultsContent() {
  ...
  const submitFeedback = useCallback(async (helpful: boolean, comment?: string) => {
    ...
    try {
      await fetch("/api/feedback", {
        ...
        body: JSON.stringify({
          analysisId: (result as AnalysisResult & { savedId?: string }).savedId,
  ...
  const handleCopy = useCallback(async (text: string) => {
    try {
      ...
      // Fallback: select text in a temporary textarea for manual Ctrl+C
